analysis_results <- function(outcome, eventdata, cohort_wide, cohort_defined_labels, exclusion, exposure, model, n_outcomes) {
	...
	temp <- cohort_wide |> 
		join(events, on="patid", how = "left", verbose=0) #collapse::join() with the default argument multiple=FALSE only joins the first matching row of the events dataset to each row of the cohort, which is intended behaviour here
	...
	if (exclusion=="outcome before indexdate") {
	  ...
	  temp <- temp |> fsubset(eval(sym(outcome))==FALSE) |> fsubset(cons_in_year_pre_index==TRUE)
...
analysis_time_updated_results <- function(outcome, eventdata, cohort_wide, cohort_defined_labels, exclusion, exposure, model, n_outcomes, time_updated_vars, time_updated_vars_spec, hes_eligible_patids, hes_dates_all) {
	#Get events
	events <- eventdata[c("patid", "obsdate")] |> 
		collapse::funique.data.frame() |> #Only keep one row if there are multiple events for the same patient on the same day
		roworder(obsdate)
...
create_cohort_long <- function(outcome, eventdata, cohort_wide, cohort_defined_labels, exclusion, n_outcomes, time_updated_vars, time_updated_vars_spec, hes_eligible_patids, hes_dates_all) {
	#Get events
	events <- eventdata[c("patid", "obsdate")] |> 
		collapse::funique.data.frame() |> #Only keep one row if there are multiple events for the same patient on the same day
		roworder(obsdate)
...
analysis_long <- function(outcome, cohort_long, cohort_defined_labels, exclusion, exposure, model, n_outcomes) {
	#Make survival object for long cohort
	survival_object <- survival::Surv(
