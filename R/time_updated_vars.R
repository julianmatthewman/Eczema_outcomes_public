...
alg_eczema_severity <- function(exposed_obs_defined, exposed_drug_defined, codelists_for_define, hes_diagnosis_eczema) {
	
	pot_top_cs_prodcodes <- codelists_for_define$full[[which(codelists_for_define$name=="topical_glucocorticoids")]] |> 
		fsubset(potency %in% c("potent", "very potent")) |> 
		pull(ProdCodeId)
	
	# eventdata_eczema <- exposed_obs_defined |> 
	# 	fsubset(medcodeid %in% codelists_for_define$codes[[which(codelists_for_define$name=="eczema")]]) |> 
	# 	rename(eventdate=obsdate)
	eventdata_phototherapy <- exposed_obs_defined |> 
		fsubset(medcodeid %in% codelists_for_define$codes[[which(codelists_for_define$name=="phototherapy")]]) |> 
		rename(eventdate=obsdate)
	# eventdata_top_cs <- exposed_drug_defined |> 
	# 	fsubset(prodcodeid %in% codelists_for_define$codes[[which(codelists_for_define$name=="topical_glucocorticoids")]]) |> 
	# 	rename(eventdate=issuedate)
	# eventdata_emollients <- exposed_drug_defined |> 
	# 	fsubset(prodcodeid %in% codelists_for_define$codes[[which(codelists_for_define$name=="emollients")]]) |> 
	# 	rename(eventdate=issuedate)
	eventdata_oral_glucocorticoids <- exposed_drug_defined |> 
		fsubset(prodcodeid %in% codelists_for_define$codes[[which(codelists_for_define$name=="oral_glucocorticoids")]]) |> 
		rename(eventdate=issuedate)
	eventdata_pot_top_cs <- exposed_drug_defined |> 
		fsubset(prodcodeid %in% codelists_for_define$codes[[which(codelists_for_define$name=="topical_glucocorticoids")]]) |> 
		fsubset(prodcodeid %in% pot_top_cs_prodcodes) |> 
		rename(eventdate=issuedate)
	eventdata_top_cn_inh <- exposed_drug_defined |> 
		fsubset(prodcodeid %in% codelists_for_define$codes[[which(codelists_for_define$name=="topical_calcineurin_inhibitors")]]) |> 
		rename(eventdate=issuedate)
	eventdata_sys_imspr <- exposed_drug_defined |> 
		fsubset(prodcodeid %in% codelists_for_define$codes[[which(codelists_for_define$name=="systemic_immunosupressants")]]) |> 
		rename(eventdate=issuedate)
	
	eventdata_hes_eczema <- hes_diagnosis_eczema |> fselect(patid, eventdate=epistart)
	...
	moderate <- bind_rows(eventdata_pot_top_cs, eventdata_top_cn_inh) |> 
		roworder(eventdate) |> 
		fselect(patid, eventdate) |> 
		ftransform(eczema_severity=2)
	
	severe <- bind_rows(eventdata_phototherapy, eventdata_sys_imspr, eventdata_hes_eczema) |> 
		roworder(eventdate) |> 
		fselect(patid, eventdate) |> 
		ftransform(eczema_severity=3)
	
	bind_rows(moderate, severe) |> 
